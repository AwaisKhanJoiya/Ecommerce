...
app.post("/login/", (req, res) => {
  user.login(
    req.body.email, // it could be email as well. Dun need to be username
    req.body.password,
    (error, user) => {
      if (error) {
        res.status(500).send();
...
app.post("/product/", (req, res) => {
  const profile_pic = req.files.file;
  const extension = profile_pic.name.split(".").pop();
  let r = (Math.random() + 1).toString(36).substring(2);
  const path = "/images/" + r + "." + extension;
  profile_pic.mv("../public" + path, (err) => {
    if (err) {
      return res.json({ error: "File cannot upload" });
    } else {
      product.insert({ ...req.body, path }, (error, result) => {
        if (error) {
          res.status(500).json(error);
        } else {
